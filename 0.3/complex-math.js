/***** Complex Number Calculation Library 0.3 *****/
...
(function (window, undefined){
  ...
  */
  ...
  function preparse(expr){
    ...
    expr = expr.replace(/i/g, "‘0,1’"); // imaginary numbers
  ...
  function getNextObject(expr, pos){
    ...
    if (expr[pos] == "‘"){
      return expr.substring(pos, expr.length).match(/^‘[0-9]+,[0-9]+’/)[0];
    ...
    throw "Error: getNextObject: Character \"" + expr[pos] + "\" not a number or an operator";
  }
  ...
  function getOperand(expr, oper, pos){
    ...
    var next = getNextObject(expr, pos);
    if (isNumber(next) || isBracket(next)){
      pos += next.length;
  ...
  // alert($.toStr(parse(preparse("-(53*3-2/(4i))+((34+53i)/(23-34i))*(-i)"))))
})(window);