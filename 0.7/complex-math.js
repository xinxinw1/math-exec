/***** Complex Number Calculation Library 0.7 *****/
...
(function (window, undefined){
  ...
  function parsePrepared(expr){
    ...
    if (isOperator(next)){
      ...
      before = parsePrepared(next.substring(1, next.length-1));
      pos += next.length;
      ...
      for (var i = name.length+1; i < next.length-1; i++){
        switch (next[i]){
          ...
          case ",": if (blevel == 1 && nlevel == 0){
            before.push(parsePrepared(next.substring(lastbound+1, i)));
            lastbound = i;
          }
        }
      }
      before.push(parsePrepared(next.substring(lastbound+1, next.length-1)));
      pos += next.length;
    } else if (next == ""){
      return "";
    } else {
